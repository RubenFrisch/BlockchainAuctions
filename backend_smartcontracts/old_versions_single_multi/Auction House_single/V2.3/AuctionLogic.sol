...
contract AuctionLogic is Cancellable {
    ...
    bool private _ownerWithdrew; //Indicates whether the owner has already withdrew or not
    ...
    function auctionIsRunning() public view virtual onlyWhenAfterStartBlock onlyWhenBeforeEndBlock returns (bool) {
        return true;
    }
        ...
        if(cancelled()) {
            ...
            if(msg.sender == owner()) { //The owner is able to withdraw the _auctionWinner's bid, which is the amount of _auctionHighestBidAmount
                if(!ownerWithdrew()) { //The owner can withdraw only once
                    ...
                    revert("TRANSACTION ERROR: Owner has already withdrew!");
                }
