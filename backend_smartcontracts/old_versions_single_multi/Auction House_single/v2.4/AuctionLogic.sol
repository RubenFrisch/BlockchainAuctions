...
contract AuctionLogic is Cancellable {
    ...
    uint256 private immutable _auctionEndBlock; //At this block number the auction ends (*passed at auction construction)
    bool private _cancellationProcessAccepted; //2-step auction cancellation mechanism
    uint256 private immutable _startingPrice; //Starting price of the auctionized item/service (optional, pass 0 as an argument to the constructor to ignore) (*passed at auction construction)
    uint256 private immutable _bidIncrement; //Bid increment (optional, pass 0 as an argument to the constructor to ignore) (*passed at auction construction)
    bool private immutable _closedAuction; //In a closed auction, only whitelisted participants are eligible of placing bids (*passed at auction construction)
    ...
    uint256 private immutable _reservePrice; //Reserve price in case when a reserve-auction is required (optional, pass 0 as an argument to the constructor to ignore) (*passed at auction construction)
    ...
    function contractETHBalance() public view virtual returns (uint256) {
        return address(this).balance;
    }

    function ownerWithdrew() public view virtual returns (bool) {
        return _ownerWithdrew;
    }

    function entryFee() public view virtual returns (uint256) {
        return _entryFee;
    }
    ...
    function hasPaidEntryFee(address participant_) internal virtual returns (bool) {
        return _entryFeesOfParticipants[participant_];
    }

    function hasWithdrawnEntryFee(address participant_) internal virtual returns (bool) {
        return _entryFeeWithdrawn[participant_];
    }

    function payEntryFee() public virtual payable onlyWhenNotOwner returns (bool) {
    ...
    function withdrawEntryFee() public virtual onlyWhenNotOwner returns (bool) {
        //Owner should not be able to pay entry fee, modifier enforces that criteria
        //We must check if the auction has already ended due to regular ending or by cancellation done by the owner, if neither of the two possible auction ending is in effect, we revert the transaction
        if(!cancelled() && auctionEndBlock() > block.number) {
            revert("TRANSACTION ERROR: withdrawing is only allowed after the auction ends or in rare cases when cancelled by the auction owner!");
