...
abstract contract OwnershipControl {
    ...
    function transferOwnership(address newOwner_) external onlyOwner returns (bool) {
        require(newOwner_ != address(0), "Transaction FAILED: The new owner cannot be the zero (burn) address!");
        require(newOwner_ != owner(), "Transaction FAILED: The new owner cannot be the current owner!");
        require(newOwner_ != pendingOwner(), "Transaction FAILED: The passed address is already the pending owner!");
