...
contract AuctionLogic is Cancellable {
    ...
    string private _ipfs; //Stores IPFS auction metadata if required for the specific use case (optional)
    AuctionERC721 private immutable _nftContractAddress; //Variable to store the auction NFT contract's address (*passed at auction construction)
    uint256 private immutable _nftTokenID; //Variable to store the tokenID of the NFT assigned to the auction (*passed at auction construction)
    ...
    )
    {
        ...
        require(isSmartContract(nftContractAddress_), "INVALID AUCTION CONTRACT INITIALIZATION: the provided NFT contract address is not a smart contract!");
        ...
        _nftContractAddress = AuctionERC721(nftContractAddress_);
        _nftTokenID = nftTokenID_;
    }

    //Check whether the contract is ERC721 compliant, its purpose is to confirm that it correctly implements the required interface for safe transfers of NFTs to avoid locking NFTs into a contract that doesn't handle NFTs
    function onERC721Received(address, address, uint256, bytes calldata) external pure returns (bytes4) {
        return bytes4(keccak256("onERC721Received(address,address,uint256,bytes)"));
    } 

    //Returns a boolean value indicating whether the given address points to a smart contract or not
    ...
    function nftContractAddress() public view returns (AuctionERC721) {
    ...
    function cancelAuction2Step() external override onlyOwner onlyWhenAfterStartBlock onlyWhenBeforeEndBlock {
        ...
        nftContractAddress().burn(nftTokenID()); //We must burn the NFT in case the auction is cancelled
    }
    ...
    function withdraw() external returns (bool) {
        ...
        if(cancelled()) {
            ...
            } else if(msg.sender == auctionWinner()) { //Thw winner of the auction is able to withdraw the NFT that stores all the auction metadata with IPFS directly to the EAO
                if(nftContractAddress().ownerOf(nftTokenID()) == address(this)) {
                    nftContractAddress().safeTransferFrom(address(this), auctionWinner(), nftTokenID()); //Transfer NFT with _nftTokenID to the winner of the auction, from the contract itself (address(this))
                    return true;
                } else {
                    revert("TRANSACTION ERROR: The NFT has been already withdrawn by the auction winnner!");
                }
