...
import "MultiSignatureGuard.sol";
...
abstract contract OwnershipController is TimelockCountdownTimerController, MultiSignatureGuard {
    ...
    event SocialGuardianRecoveryInitiated(address indexed signer_);
    ...
    function socialGuardianRecovery(address newOwner_) external onlySigner timelocked(block.timestamp) multiSignatureGuard returns (bool) {
        ...
        _resetPendingOwner();
        emit SocialGuardianRecoveryInitiated(msg.sender);
        return true;
    }
