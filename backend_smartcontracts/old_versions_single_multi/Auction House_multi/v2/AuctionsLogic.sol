...
contract AuctionsLogic is Cancellable {
    ...
    modifier onlyBeforeStartBlock(bytes32 auctionID_) {
        require(block.number < _auctionStartBlock[auctionID_], "The auction has already started!");
        _;
    }
    ...
    // <<<< Whitelisting functionality for closed (private/whitelist) auctions >>>>
    function closedAuction(bytes32 auctionID_) public view returns (bool) {
        return _closedAuction[auctionID_];
    }

    function configureClosedAuction(bytes32 auctionID_) external onlyOwner onlyIfAuctionExists(auctionID_) onlyBeforeStartBlock(auctionID_) returns (bool) {
