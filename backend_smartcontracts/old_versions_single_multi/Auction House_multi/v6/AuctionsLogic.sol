...
contract AuctionsLogic is Cancellable {
        ...
        returns (bool) 
    {
        ...
        require(isSmartContract(nftContractAddress_), "Address is not a contract!");
        require(AuctionERC721(nftContractAddress_).supportsInterface(type(IERC721).interfaceId), "Contract does not implement IERC721!");
        require(AuctionERC721(nftContractAddress_).ownerOf(nftTokenID_) == address(this),"NFT must be transferred to the auction contract first!");
        ...
        returns (bool) 
    {
        ...
        if(auctionHighestBidAmount(auctionID_) != 0) {
            require(totalBid >= (auctionHighestBidAmount(auctionID_) + bidIncrement(auctionID_)), "Bid must exceed the highest bid + increment!");
        }
    ...
    function cancelAuction(bytes32 auctionID_) 
        ...
        returns (bool)
    {
        require(isCancellable(auctionID_), "This auction is not cancellable!");
