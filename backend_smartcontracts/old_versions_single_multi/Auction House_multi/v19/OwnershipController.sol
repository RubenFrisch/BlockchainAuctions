...
import "TimelockCountdownTimerController.sol";
...
abstract contract OwnershipController is TimelockCountdownTimerController {
    ...
    function startQueue() external override onlyOwner returns (bool) {
        super._startQueue();
        return true;
    }
    ...
    function transferOwnership(address newOwner_) external onlyOwner timelocked(block.timestamp) returns (bool) {
    ...
    function startRenounceProcess() external onlyOwner timelocked(block.timestamp) returns (bool) {
