...
contract AuctionsLogic is Cancellable {
    ...
    modifier onlyIfAuctionExists(bytes32 auctionID_) {
        require(auctionExists(auctionID_), "Auction does not exist with this ID!");
        _;
    }
    ...
    modifier onlyIfAuctionDoesNotExist(bytes32 auctionID_) {
        require(!auctionExists(auctionID_), "Auction already exists with this ID!");
        _;
    }
    ...
    modifier onlyAfterStartBlock(bytes32 auctionID_) {
        require(auctionStartBlock(auctionID_) <= block.number, "The auction has not started yet!");
        _;
    }
    ...
    modifier onlyBeforeEndBlock(bytes32 auctionID_) {
        require(block.number <= auctionEndBlock(auctionID_), "The auction has already ended!");
        _;
    }
    ...
    modifier onlyBeforeStartBlock(bytes32 auctionID_) {
        require(block.number < auctionStartBlock(auctionID_), "The auction is running!");
        _;
    }

    // <<< AUCTION BUILDER FUNCTION >>>
    /// @dev Core function that enables the creation of new parametric auctions
    /// @notice Core function that enables the creation of new parametric auctions
    /// @param auctionID_ The 256 bit hash identifier of the auction to be created, function reverts if there is an auction already created with the same auctionID_
    /// @param auctionStartBlock_ The block number where the auction will be considered commenced
    /// @param auctionEndBlock_ The block number where the auction will be considered concluded
    /// @param startingPrice_ The starting price in Wei of the good or service being sold
    /// @param bidIncrement_ The amount of Wei that the next bid must be higher than the previous highest bid amount in order to be accepted as the winning bid (except for the first bid)
    /// @param reservePrice_ The minimum amount of Wei that the highest bid must reach for the good or service to be sold to the winner (also called reservation price)
    ...
    /// @param auctionSnipeBlocks_ The number of additional blocks to be added to the ending block number in case of a bid that triggers the snipe prevention mechansim, extending the auction's duration
    /// @param nftContractAddress_ The address of the IERC-721 complient NFT smart contract that the implementation auction contract will interact with to manage the ownership of the NFT that represents the good or service
    /// @param nftTokenID_ The ID number of the NFT token that represents the good or service to be sold in the specific auction
        ...
        returns (bool) 
    {
        ...
        require(AuctionERC721(nftContractAddress_).ownerOf(nftTokenID_) == address(this),"NFT must be transferred to the auction smart contract before the auction can be created!");
    ...
    /**
     * @dev This function allows a user to attempt placing a bid on an auction.
     */
        ...
        returns (bool) 
    {
        ...
        if(auctionHighestBidAmount(auctionID_) != 0) {
            require(totalBid >= (auctionHighestBidAmount(auctionID_) + bidIncrement(auctionID_)), "Bid must exceed the highest bid + bid increment!");
        }
        ...
        returns (bool) 
    {
        ...
        if(isCancelled(auctionID_) || (auctionHighestBidAmount(auctionID_) < reservePrice(auctionID_))) {
            ...
            require(!ownerWithdrew(auctionID_), "Owner has already withdrawn the winning bid!");
            ...
            require(nftContractAddress(auctionID_).ownerOf(nftTokenID(auctionID_)) == address(this), "NFT has been already withdrawn by the auction winner!");
