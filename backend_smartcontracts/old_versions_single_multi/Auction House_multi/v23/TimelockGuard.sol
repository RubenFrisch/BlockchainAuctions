...
abstract contract TimelockGuard {
     ...
     // <<< STATE VARIABLES >>>
    /// @dev Constant that stores the time duration of the timelock delay (can be changed, no side-effects)
    uint256 private constant _DELAY = 10 days;

    /// @dev Constant that stores the time duration of the grace period (can be changed, no side-effects)
